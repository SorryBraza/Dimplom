        ...
        if {'first_name', 'last_name', 'email', 'password', 'company', 'position'}.issubset(self.request.data):
            try:
                ...
                if user_serializer.is_valid():
                    ...
                    send_email('Confirmation of registration', f'Your confirmation token {token.key}',
                               user.email)
...
class PartnerUpdate(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if url:
            try:
                task = get_import(request.user.id, url)
            except IntegrityError as e:
                return JsonResponse({'Status': False,
...
class PartnerOrders(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        send_email('Order status update', 'The order has been processed', request.user.email)
        return Response(serializer.data)
        ...
        if {'id', 'contact'}.issubset(request.data):
            if request.data['id'].isdigit():
                try:
                    ...
                    if is_updated:
                        send_email('Order status update', 'The order has been formed ',
                                   request.user.email)
                        return JsonResponse({'Status': True}, status=status.HTTP_200_OK)
